...
-   Numerous substantive difficulties with entries that do not map neatly:
    -   [*\*aghlu-*](https://indo-european.info/pokorny-etymological-dictionary/whnjs.htm) *(IEW*, p. 8) 'dark cloud; rainy weather': new hyponym (of both 'cloud' and 'weather') needed
    -   [*\*ab-*](https://indo-european.info/pokorny-etymological-dictionary/whnjs.htm) *(IEW*, p. 1) 'water, river': new hypernym needed? or two mappings?
-   The general idea of English (or any other language) **↦** PIE is flawed, because PIE ≠ English (etc.)!
...
-   Second approach is one more in line with what is done in mainstream NLP – identifying semantic relationships through word embeddings.
...
-   We use existing aligned models of Spanish & French ([Joulin et al., 2018](https://arxiv.org/pdf/1804.07745 "Loss in Translation: Learning Bilingual Word Mapping with a Retrieval Criterion")) as a source of vector and word information
    -   Aligned word vectors taken from Facebook AI Research's fastText ([Bojanowski et al., 2017](https://arxiv.org/pdf/1607.04606 "Enriching Word Vectors with Subword Information"))
-   Words filtered out:
...
1.  If there isn't a 1:1 correspondence, identify the centroid of lemma's vector: **language-word center**
...
-   This is not exactly a bad thing, as this is the case for most languages in our discipline!
...
-   Same problems identified previously remain

# Future Directions: using LLM models

-   Probably the best course of action is to stick with the current strategy, but:

    -   Utilize LLMs (such as GPT) for modelling (hyperspace) for greater precision and differentiation of polysemous forms

    -   Instead of Google Translate, use bilingual dictionary or LLM

    -   Add additional Romance languages; when happy, move on to other subbranches (likely Slavic or Indic)
